...
def api_clientes():
    ...
    if request.method == 'GET':
        ...
        cliente = Cliente(
            ...
            cidade=sanitize_input(data.get('cidade', '')),
            estado=sanitize_input(data.get('estado', '')),
            cep=sanitize_input(data.get('cep', '')),
            user_id=user_id
        )
...
def api_categorias():
    if request.method == 'GET':
        ...
        # Criar nova categoria (salvar no banco seria ideal, mas por simplicidade vamos apenas retornar sucesso)
        data = request.get_json()
        nova_categoria = data.get('categoria', '').strip()
        
        if nova_categoria:
            # Aqui você poderia salvar em uma tabela de categorias dedicada
